...
function useOnline() {
  const [isOnline, setIsOnline] = (0, import_react.useState)(typeof navigator !== "undefined" ? navigator.onLine : false);
...
var AuthConfigManager = (_a = class {
  ...
  static getInstance() {
    if (!_a.instance) _a.instance = new _a();
    return _a.instance;
  }
...
async function getCsrfToken() {
  var _a2;
  const { baseUrl, basePath, credentials } = authConfigManager.getConfig();
  return ((_a2 = await fetchData("csrf", {
    baseUrl,
    basePath,
    credentials
  }, logger)) == null ? void 0 : _a2.csrfToken) ?? "";
}
...
async function signIn(provider, options = {}, authorizationParams = {}) {
  ...
  if (res.ok) await ((_a2 = config.fetchSession) == null ? void 0 : _a2.call(config, { event: "storage" }));
  return {
    error,
    status: res.status,
    ok: res.ok,
  ...
  (0, import_react.useEffect)(() => {
    const handleMessage = (event) => {
      ...
      if (event.data.status) {
        setState(event.data);
        if (event.data.status === "success") onSuccess == null ? void 0 : onSuccess();
